...
lambda1 <- 0.1; lambda2 <- 0.5; lambda3 <- 1
...
for(irep in 1:ntot){
  ...
  mapply(\(x.c, x.p, i, kappa.int){
    ...
    # Accept Rule
    R.num <- p.p * ll.p *  dlnorm(x.p * x.c, x.c, kappa.int)
    R.den <- p.c * ll.c *  dlnorm(x.p * x.c, x.c, kappa.int)
   
    R <- min(1, R.num / R.den, na.rm = TRUE)
    ...
    # if (irep < 0.5 * nburn) {
    #   if(accept[i] / irep > 0.3) kappa[i] <<- kappa.int * 1.01
    #   if(accept[i] / irep < 0.15) kappa[i] <<- kappa.int * 0.99
    # }
    
    
  }, lambda.draw, lambda.prop, 1:3, kappa)
