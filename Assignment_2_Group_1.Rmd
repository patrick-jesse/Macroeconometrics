...
for(irep in 1:ntot){
  ...
  # Step 0 : MH Step for lambdas
  
  # Draw proposal from log normal
  lambda.prop <- exp(lambda.draw + kappa * rnorm(3))
  ...
  lapply(list(V_prior.c, V_prior.p), function(x){
    V_prior <- x
      ...
      for(pp in 1:plag){ # over all lags
        for(kk in 1:M){ # over all coefficients
          ...
          if(mm==kk){
            ...
            V_prior[(pp-1)*M+kk,(pp-1)*M+kk,mm] <- (sigs[mm]/sigs[kk])*(lambda2/pp)^2
          }
        }
      }
...
for(irep in 1:ntot){
  ...
  # Draw proposal from log normal
  lambda.prop <- exp(c(lambda1, lambda2, lambda3) + kappa * rnorm(3))
