...
lambda1 <- 0.1; lambda2 <- 0.2; lambda3 <- 1
...
for(irep in 1:ntot){
  ...
  lambda.prop <- mapply(\(x, y) rlnorm(1, log(x), y), lambda.draw, kappa)
  ...
  mapply(\(x.c, x.p, i){
    ...
    # Accept Rule
    R.num <- p.p * ll.p *  dlnorm(x.p * x.c, x.c, kappa[i])
    R.den <- p.c * ll.c *  dlnorm(x.p * x.c, x.c, kappa[i])
