...
for(irep in 1:ntot){
  ...
  lambda.prop <- mapply(\(x, y) rlnorm(1, x, y), lambda.draw, kappa)
  ...
  ll.c <- mvtnorm::dmvnorm(c(tmp.store[[1]][["A_draw"]]),
                           a_prior,
                           tmp.store[[1]][["V_underscore"]])
  
  ll.p <- mvtnorm::dmvnorm(c(tmp.store[[2]][["A_draw"]]),
                           a_prior,
                           tmp.store[[2]][["V_underscore"]])
  ...
  mapply(\(x.c, x.p, i){
    ...
    # EValuate prior densities
    p.c <- dgamma(x.c, alpha.hyper, beta.hyper)
    p.p <- dgamma(x.p, alpha.hyper, beta.hyper)
    
    # Accept Rule
    R.num <- p.p * ll.p *  dlnorm(x.c, x.p, kappa[i])
    R.den <- p.c * ll.c *  dlnorm(x.p, x.c, kappa[i])
