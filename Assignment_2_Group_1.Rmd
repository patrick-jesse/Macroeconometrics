...
Before integrating it into the VAR code, let's first try to code up the MH-RW algorithm for the lambda priors.
...
lambda1 <- 0.1; lambda2 <- 0.2; lambda3 <- 0.1
...
for(irep in 1:ntot){
  ...
  lambda.prop <- rlnorm(3, lambda.draw, kappa)
  ...
  # Aceppt or reject
  R.num <- prod(p.p) * ll.p *  prod(dlnorm(lambda.prop * lambda.draw, lambda.draw, kappa))
  R.den <- prod(p.c) * ll.c *  prod(dlnorm(lambda.prop * lambda.draw, lambda.draw, kappa))
